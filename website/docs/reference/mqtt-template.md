...
:::note Example
As an *example*, we assume that we want the Eval Kit device to connect to Azure IoT Central.

For this, we connect devices whose model type is `sy-epc-rpi30` (the device model of the Eval Kit).
...
This would match not only the current Raspberry Pi 3-based Eval Kit, but also the new Raspberry Pi 4 devices.
